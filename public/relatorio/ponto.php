...
<body>
  ...
  <div class="container">
    ...
    <div class="modal_ponto" id="modal_ponto" title="Justificar ponto!">
      <p>
      <form>
        <h3>Ajuste os horários conforme nescessário!</h3>
        <p class="externo">
          <label for="acao">Ação:</label>
          <input type="text" name="acao" id="acao" placeholder="Acão que deverá ser executada pelo RH.">
        </p>
        <p class="interno">
        ...
        <p class="externo">
          <label for="justificativa">Jutificativa:</label>
          <input type="text" placeholder="Justificativa para a alteração do ponto!" id="justificativa"
            name="justificativa">
        </p>
  ...
  <script>
      ...
      $("#modal_ponto").dialog({
        ...
        buttons: {
          "Enviar": function () {
            ...
            var hora_ida_lanche = $('#hora_ida_lanche').val()
            var hora_volta_lanche = $('#hora_volta_lanche').val()
            var hora_saida = $('#hora_saida').val()
            var justificativa = $('#justificativa').val()
            var acao = $('#acao').val()
            var matricula = $('#matricula_modal').val()
            var nome = $('#nome_modal').val()
            var escala = $('#escala_modal').val()
            ...
            if (acao === '') {
            ...
            } else if (hora_entrada === '' && hora_ida_lanche === '' && hora_volta_lanche === '' && hora_saida === '') {
              swal({
                title: "Erro!",
                text: "Insira ao menos um horário!",
                icon: "error",
                button: false,
                timer: 1500
              });
              ...
              $.ajax({
                url: '/archerx/public/funcoes/api/glpi_api.php',
                method: 'POST',
                data: {
                  tipo: 'chamado',
                  hora_entrada: hora_entrada,
                  hora_ida_lanche: hora_ida_lanche,
                  hora_volta_lanche: hora_volta_lanche,
                ...
                success: async function (response) {
                  ...
                  $('#modal_ponto').dialog("close"); // Fechar o modal sem salvar
            ...
            $.ajax({
              ...
              success: async function (response) {
                if (response['erro'] == false) {
                  ...
                  $('#modal_glpi').dialog("close"); // Fechar o modal após salvar
    ...
    $("#tabela_ponto tbody").on('click', 'tr', function () {
      ...
      if (cookie_glpi) {
        ...
        console.log("Valores a enviar ao modal: ", matricula, nome, escala, data);
