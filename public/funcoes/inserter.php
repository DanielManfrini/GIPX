...
elseif (isset($_POST['qd'])) {
    $newqd=mysqli_fetch_assoc(select('select id from archerx.qdu where qdu_serial="'.$_POST['qd'].'"'));
    if ($newqd!=""){
        echo "<script language='javascript' type='text/javascript'>
        alert('QD já existe!');window.location;history.go(-1);</script>";
    } else {
        $insert_qdu = 'INSERT INTO archerx.qdu (qdu_serial) VALUES ("'.$_POST['qd'].'")';
        if(insert($insert_qdu)=='sucesso'){
            echo "<script language='javascript' type='text/javascript'>
            alert('QD cadastrada com sucesso!');window.location;history.go(-1);</script>";
        } else {
            echo "<script language='javascript' type='text/javascript'>
            alert('Erro ao cadastrar QD!');window.location;history.go(-1);</script>";
        }
    }    
}
else{
