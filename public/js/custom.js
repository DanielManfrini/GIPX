async function buscar_usuario(registro){
    ...
    if (valor_matricula.length == 6){
        ...
        const dados = await fetch('http://172.10.20.60/archerx/public/buscar_usuario.php?matricula=' + valor_matricula);
        // ler os dados
...
async function busca_cartao_provisorio(){
    ...
    console.log(valor_matricula)
    // realizar a requisição
    const dados_provisorio = await fetch('http://172.10.20.60/archerx/public/busca_provisorio.php?matricula=' + valor_matricula);
    // ler os dados
...
async function associar_cartao(){
    
    // COLETANDO AS VARIÁVEIS
    var matricula = document.querySelector('#matricula');
    var cartao = document.querySelector('#cartao_provisorio');
    var data_inicial = document.querySelector('#data_inicial');
    var data_final = document.querySelector('#data_final');

    var valor_matricula = matricula.value;
    var valor_cartao = cartao.value;
    var valor_data_inicial = data_inicial.value;
    var valor_data_final = data_final.value;

    var dados_cartao_provisorio = [valor_matricula,valor_cartao ,valor_data_inicial ,valor_data_final];
    console.log(dados_cartao_provisorio);
    

    // realizar a requisição
    const dados = await fetch('http://172.10.20.60/archerx/public/funcoes.php?dados_cartao_provisorio='+dados_cartao_provisorio);
    // ler os dados
    const resposta = await dados.json();
    console.log(resposta);
}